...
export class SongDisplayByGenre extends React.Component {
    ...
    componentDidMount = (props) => {
        ...
        if(this.props.genreName !== null){
            var encodedGenreString = encodeURI(this.props.genreName);
            axios.get(`/songs/for/genre?genre=${encodedGenreString}`) // place /songs/for/genre?genre={} endpoint here
            .then(function(response1) {
                var songUrlList = [];
                var promises = [];
                for(const dataObject of response1.data){
                    var encodedArtistString = encodeURI(dataObject.artistSK);
                    var encodedSongString = encodeURI(dataObject.songPK);
                    promises.push(
        ...
        <div>
            <div>
                <button 
                    ...
                    onClick={()=>{this.props.changeGenreViewState("genre_selection")}}>
...
export default SongDisplayByGenre;